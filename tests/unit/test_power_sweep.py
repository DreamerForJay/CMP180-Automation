...
class SweepBackend:
    ...
    def configure(self, plan): self.power = plan.generator_power_dbm; self.calls.append(("configure", self.power))
    def rf_on(self): self.calls.append(("rf_on", self.power))
    def initiate_single(self): self.calls.append(("initiate", self.power))
    def wait_ready(self):
        self.calls.append(("wait", self.power))
        if self.power == self.fail_power_dbm: raise TimeoutError("point timeout")
    def fetch_result(self): return {"generator_power_dbm": self.power, "evm_all_carriers_db": -36.0}
    def stop_measurement(self): self.calls.append(("stop", self.power))
    def rf_off(self): self.calls.append(("rf_off", self.power))
...
def sweep_plan(**overrides):
    ...
    values = dict(single=single, start_power_dbm=-50.0, stop_power_dbm=-40.0, step_power_dbm=5.0, dwell_time_s=.1)
