...
## 13. 固定四點功率掃描 HIL（需逐次現場確認）

探索已確認 -60 dBm 在目前設定下回傳 `INV`，因此正式候選入口固定 6105 MHz、320 MHz、expected power -20 dBm，依序從 -55 dBm 提高至 -40 dBm。執行前重新確認 Generator `OFF`、measurement `OFF` 或 `RDY`、error queue empty，以及 RF1.1 → RF1.5 單一 cable 直連、無衰減器、操作員在現場。
...
## 14. Fixed four-point power-sweep HIL (per-run on-site confirmation required)

Exploration confirmed that -60 dBm returns `INV` with the current configuration. The candidate entry point is therefore fixed to 6105 MHz, 320 MHz, -20 dBm expected power, and increases from -55 dBm to -40 dBm. Before execution, reconfirm Generator `OFF`, measurement `OFF` or `RDY`, an empty error queue, the single direct RF1.1-to-RF1.5 cable, no attenuator, and operator presence.
