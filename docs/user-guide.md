...
量測紀錄區分載入中、空清單、無符合條件與更新失敗；失敗會保留上次資料並提示重新整理。唯讀紀錄請求逾時為 15 秒。錯誤通知保留至手動關閉；新通知會取代舊通知。結果頁明確區分無資料、完整、部分／取消及全部無效，並提醒完成不等於規格通過；部分結果會同時列出有效與無效／未完成點數。亮／暗主題共用提示配色，中英切換更新紀錄狀態、通知、結果摘要及圖表說明；原始診斷內容保留原文。切換不重新送出量測或 RF 請求。
