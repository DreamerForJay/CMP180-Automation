...
| 圖 | 內容 | 來源 |
|---|---|---|
| [系統架構](system-architecture.html) | 從操作員瀏覽器到 CMP180 實機的完整分層、RF 授權區，以及不進 RF 路徑的 Constellation Mock／artifact 流向 | [`system-architecture.json`](system-architecture.json) |
...
系統架構圖的既有節點以 `SRC` 徽章指向 revision `7f87d2d` 當下真實存在的檔案與行號，
共 8 筆原始碼引用，由 Archify 對本 repo 驗證通過後才產生。新 Constellation 節點則明確標示
`HIL PENDING`；其來源仍是本輪未提交的工作樹，待提交後才能綁定不可變 revision：
...
- `src/cmp180_evm/constellation/` — 純軟體 I/Q model、Mock、分析與五種 artifact
