"""Run the ten-point, 2 dB-step CMP180 WLAN power-sweep HIL.

Extends the earlier four-point (-55/-50/-45/-40 dBm) HIL with finer
resolution across the same known-valid range. Deliberately starts at
-58 dBm rather than the envelope's -60 dBm floor: -60 dBm was already
confirmed twice (2026-08-20, docs/hardware-discovery.md) to return `INV`
at the current expected-power/trigger configuration, and
workflow/power_sweep.py stops the whole sweep on the first invalid point
-- starting there would abort before any of the other nine points ran.
"""
...
from cmp180_evm.results.artifacts import save_power_sweep_result
...
POWERS_DBM = (-58.0, -56.0, -54.0, -52.0, -50.0, -48.0, -46.0, -44.0, -42.0, -40.0)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--confirm-ten-point-power-sweep", action="store_true")
...
def _emergency_cleanup(instrument: RsInstrument, registry) -> tuple[str, str, list[str]]:
    ...
    try:
        ...
        # STOP 失敗時才送 ABORt，但任何情況都不能跳過 RF Off。
        try:
...
def main() -> int:
    ...
    if not (
        ...
        and args.confirm_ten_point_power_sweep
    ):
    ...
    try:
        ...
        result = run_power_sweep(backend, plan)
        points = [
            {"generator_power_dbm": power_dbm, **point.values}
            for power_dbm, point in zip(result.requested_powers_dbm, result.points)
        ]
        ...
        for index, (power_dbm, point) in enumerate(
        ...
        if not result.completed:
            print(f"Sweep stopped at {result.failed_power_dbm}: {result.error}", file=sys.stderr)
            exit_code = 3
    except Exception as exc:
        print(f"Real power sweep failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        exit_code = 3
    finally:
