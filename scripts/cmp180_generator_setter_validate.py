"""Validate CMP180 Generator setters by writing back current values with RF off."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Confirm writing the currently read frequency and level back while RF is OFF.",
    )
...
def main() -> int:
    ...
    try:
        ...
        result = validate_same_value_setters(instrument, registry)
        for check in result.checks:
            print(
                f"PASS {check.name:10} original={check.original:g} "
                f"readback={check.readback:g} [{check.error}]"
            )
        print(f"PASS RF state   {result.initial_rf_state} -> {result.final_rf_state}")
    except Exception as exc:
        print(f"Setter validation failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 3
    finally:
